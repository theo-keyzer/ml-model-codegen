...
# Includes PatternAware extension for oscillation detection.
import jax
...
class SARConfig:
    ...
    def __post_init__(self):
        ...
        if self.strategy_weights is None:
            # Default weights: Balanced, with room for jumps (UPDATED: Lower jump weights to reduce spamming)
            self.strategy_weights = {
                "PERTURB_BEST": 0.15, "BEST_PARAMS": 0.15, "RANDOM_RESTART": 0.25,
                "GRADIENT_ESCAPE": 0.2, "AVOIDANCE_RESTART": 0.2,
                "QUANTUM_JUMP": 0.025, "NUCLEAR_JUMP": 0.025  # Lower for less frequent jumps
            }
...
class SARMemoryManager:
    ...
    def _compile_functions(self):
        ...
        def select_strategy(steps_since_improvement, reset_patience, strategy_effectiveness, key, enable_jumps, jump_thresh):
            ...
            # UPDATED: Reduce jump weights significantly (as recommended in analysis)
            base_weights = jnp.array([0.15, 0.15, 0.25, 0.2, 0.2, 0.025, 0.025])  # Indices 0-6: Lower jumps (5,6)
            
            severity_weights = jnp.where(
                severity > jump_thresh, 
                base_weights * jnp.array([0.1, 0.1, 0.2, 0.2, 0.2, 0.1, 0.1]),  # Mild boost for jumps on high severity  
                base_weights  
            )
...
# NEW: PatternAware Extension (User-Provided Enhancement 3)
class PatternAwareSARMemoryManager(SARMemoryManager):
    """Extended memory manager that recognizes optimization patterns."""
    
    def __init__(self, problem_dim: int, config: SARConfig):
        super().__init__(problem_dim, config)
        self.oscillation_patterns: List[float] = []  # Track distance patterns
        self.cycle_detector = self._create_cycle_detector()  # Internal detector
    
    def _create_cycle_detector(self):
        """Simple cycle detector based on distance patterns."""
        class CycleDetector:
            def __init__(self, threshold: float = 0.05, min_cycles: int = 3):
                self.threshold = threshold
                self.min_cycles = min_cycles
                self.patterns: List[float] = []
            
            def is_oscillating(self, distances: List[float]) -> bool:
                if len(distances) < self.min_cycles * 2:
                    return False
                self.patterns.extend(distances)
                # Check if distances repeat (simple autocorrelation-like)
                recent = self.patterns[-len(distances)*2:]  # Last 2x distances
                for i in range(self.min_cycles, len(recent) // 2):
                    cycle_len = i
                    if all(abs(recent[j] - recent[j + cycle_len]) < self.threshold 
                           for j in range(len(recent) - cycle_len)):
                        return True
                return False
        return CycleDetector()
    
    def detect_oscillation_pattern(self, current_params: jnp.ndarray) -> bool:
        """Detect if optimizer is oscillating between similar points."""
        if self.stuck_points_ptr[0] < 3:
            return False
        
        # Get recent points as NP for simplicity
        recent_points = np.array(self.stuck_points_fifo[:3])  # Last 3
        distances = [np.linalg.norm(recent_points[i] - current_params) for i in range(3)]
        self.oscillation_patterns.extend(distances)
        
        return self.cycle_detector.is_oscillating(distances)
    
    def get_intelligent_reset(self, current_params, best_params, gradient, 
                              steps_since_improvement, reset_patience: int, pattern_detected: bool = False):
        """Smarter reset strategy based on detected patterns."""
        if pattern_detected:
            # If oscillating, use more aggressive reset (lower patience)
            effective_patience = max(5, reset_patience // 5)  # At least 5
            print("🔄 Oscillation detected – Aggressive reset triggered!")
            return self.perform_reset(current_params, best_params, gradient,
                                      steps_since_improvement, effective_patience)
        else:
            return self.perform_reset(current_params, best_params, gradient,
    ...
    def perform_reset(self, current_params: jnp.ndarray, best_params: jnp.ndarray,
                      gradient: jnp.ndarray, steps_since_improvement: int, reset_patience: int) -> Dict[str, Any]:
        """Override to include pattern detection."""
        # NEW: Detect pattern before reset
        pattern_detected = self.detect_oscillation_pattern(current_params)
        
        # Delegate to intelligent reset (avoids recursion; this calls base perform_reset indirectly)
        return self.get_intelligent_reset(current_params, best_params, gradient, 
                                          steps_since_improvement, reset_patience, pattern_detected)

# NEW: Factory for PatternAware
def create_pattern_aware_manager(problem_dim: int, config: SARConfig) -> PatternAwareSARMemoryManager:
    """Factory to create PatternAware manager."""
    return PatternAwareSARMemoryManager(problem_dim, config)

# Enhanced test (covers custom strategy, unique points, clear, jumps, and patterns)
def test_sar_memory():
    print("\n🧠 Enhanced Test: SARMemoryManager API v2.2 (with Jumps & Patterns)")
    config = SARConfig(spf_depth=3, avoidance_strength=0.8, enable_jumps=True, quantum_jump_range=4.0)
    manager = create_pattern_aware_manager(problem_dim=2, config=config)  # Use aware version
    ...
    # Test pattern detection (simulate oscillation)
    print(f"Pattern detected? {manager.detect_oscillation_pattern(jnp.array([0.9, 0.9]))}")

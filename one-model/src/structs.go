...
func (me KpCanon) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if (va[0] == "Link_concept" && len(va) > 1) { // one.unit:25, go-struct-rio.act:706
	...
	if (va[0] == "Link_relation" && len(va) > 1) { // one.unit:26, go-struct-rio.act:706
		for _, st := range glob.Dats.ApLink {
	...
	if (va[0] == "Objective_canonical_form" && len(va) > 1) { // one.unit:44, go-struct-rio.act:706
	...
	if (va[0] == "Memory_canonical_form" && len(va) > 1) { // one.unit:68, go-struct-rio.act:706
	...
	if (va[0] == "Thought_canonical_form" && len(va) > 1) { // one.unit:95, go-struct-rio.act:706
	...
	if (va[0] == "Memo_canonical_form" && len(va) > 1) { // one.unit:112, go-struct-rio.act:706
	...
	if va[0] == "previous" { // one.unit:8, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApCanon[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Canon > one.unit:8, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Canon > one.unit:8, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
...
func (me KpCanon) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "Link" { // one.unit:20, go-struct-rio.act:685
	...
	if va[0] == "Section" { // one.unit:28, go-struct-rio.act:685
	...
	if (va[0] == "Link_concept") { // one.unit:25, go-struct-rio.act:595
		for _, st := range glob.Dats.ApLink {
			if (st.Kconceptp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Link_relation") { // one.unit:26, go-struct-rio.act:595
		for _, st := range glob.Dats.ApLink {
			if (st.Krelationp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Objective_canonical_form") { // one.unit:44, go-struct-rio.act:595
		for _, st := range glob.Dats.ApObjective {
			if (st.Kcanonical_formp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Memory_canonical_form") { // one.unit:68, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemory {
			if (st.Kcanonical_formp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Thought_canonical_form") { // one.unit:95, go-struct-rio.act:595
		for _, st := range glob.Dats.ApThought {
			if (st.Kcanonical_formp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Memo_canonical_form") { // one.unit:112, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemo {
			if (st.Kcanonical_formp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Canon %s,%s > one.unit:8, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func (me KpLink) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Link > one.unit:20, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpLink) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "parent" { // one.unit:8, go-struct-rio.act:570
		if me.Kparentp >= 0 {
	...
	if va[0] == "relation" {
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Link %s,%s > one.unit:20, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadSection(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApCanon[ len( act.ApCanon )-1 ].Childs = append(act.ApCanon[ len( act.ApCanon )-1 ].Childs, st)
	act.ApCanon[ len( act.ApCanon )-1 ].ItsSection = append(act.ApCanon[ len( act.ApCanon )-1 ].ItsSection, st)	// one.unit:8, go-struct-rio.act:416
	name,_ := st.Names["name"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Section_" + name	// one.unit:32, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpSection) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	...
	if va[0] == "previous" { // one.unit:28, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApSection[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Section > one.unit:28, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Section > one.unit:28, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpSection) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "parent" { // one.unit:8, go-struct-rio.act:570
		...
		if va[0] == "level" && len(va) > 1 && me.Kparentp >= 0 { // one.unit:33, go-struct-rio.act:233
			pos := 0
	        ...
	        fmt.Printf("?No its %s for Section %s,%s > one.unit:28, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func (me KpObjective) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if va[0] == "canonical_form" { // one.unit:44, go-struct-rio.act:621
		if (me.Kcanonical_formp >= 0 && len(va) > 1) {
			return( glob.Dats.ApCanon[ me.Kcanonical_formp ].GetVar(glob, va[1:], lno) )
		}
	}
	if (va[0] == "Memo_objective_id" && len(va) > 1) { // one.unit:113, go-struct-rio.act:706
		for _, st := range glob.Dats.ApMemo {
	...
	if va[0] == "previous" { // one.unit:41, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApObjective[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Objective > one.unit:41, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Objective > one.unit:41, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpObjective) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "Memory" { // one.unit:61, go-struct-rio.act:685
	...
	if va[0] == "Thought" { // one.unit:78, go-struct-rio.act:685
	...
	if (va[0] == "Memo_objective_id") { // one.unit:113, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemo {
			if (st.Kobjective_idp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Objective %s,%s > one.unit:41, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadMemory(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApObjective[ len( act.ApObjective )-1 ].ItsMemory = append(act.ApObjective[ len( act.ApObjective )-1 ].ItsMemory, st)	// one.unit:41, go-struct-rio.act:416
	name,_ := st.Names["memory_id"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Memory_" + name	// one.unit:62, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpMemory) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if va[0] == "invalidated_by" { // one.unit:67, go-struct-rio.act:621
		if (me.Kinvalidated_byp >= 0 && len(va) > 1) {
			return( glob.Dats.ApThought[ me.Kinvalidated_byp ].GetVar(glob, va[1:], lno) )
		}
	}
	if va[0] == "canonical_form" { // one.unit:68, go-struct-rio.act:621
		if (me.Kcanonical_formp >= 0 && len(va) > 1) {
			return( glob.Dats.ApCanon[ me.Kcanonical_formp ].GetVar(glob, va[1:], lno) )
		}
	}
	if (va[0] == "parent") { // one.unit:41, go-struct-rio.act:585
		if (me.Kparentp >= 0 && len(va) > 1) {
			return( glob.Dats.ApObjective[ me.Kparentp ].GetVar(glob, va[1:], lno) );
		}
	}
	if va[0] == "previous" { // one.unit:61, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApMemory[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Memory > one.unit:61, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Memory > one.unit:61, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpMemory) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "parent" { // one.unit:41, go-struct-rio.act:570
		if me.Kparentp >= 0 {
	...
	if (va[0] == "Memo_memory_id") { // one.unit:114, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemo {
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Memory %s,%s > one.unit:61, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadThought(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApObjective[ len( act.ApObjective )-1 ].Childs = append(act.ApObjective[ len( act.ApObjective )-1 ].Childs, st)
	act.ApObjective[ len( act.ApObjective )-1 ].ItsThought = append(act.ApObjective[ len( act.ApObjective )-1 ].ItsThought, st)	// one.unit:41, go-struct-rio.act:416
	name,_ := st.Names["thought_id"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Thought_" + name	// one.unit:79, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpThought) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if va[0] == "canonical_form" { // one.unit:95, go-struct-rio.act:621
		if (me.Kcanonical_formp >= 0 && len(va) > 1) {
			return( glob.Dats.ApCanon[ me.Kcanonical_formp ].GetVar(glob, va[1:], lno) )
		}
	}
	...
	if (va[0] == "Memory_invalidated_by" && len(va) > 1) { // one.unit:67, go-struct-rio.act:706
		for _, st := range glob.Dats.ApMemory {
	...
	if va[0] == "previous" { // one.unit:78, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApThought[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Thought > one.unit:78, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Thought > one.unit:78, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpThought) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "Memo" { // one.unit:108, go-struct-rio.act:685
	...
	if (va[0] == "Memory_invalidated_by") { // one.unit:67, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemory {
			if (st.Kinvalidated_byp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
		...
		if va[0] == "tree_linear" && len(va) > 1 && me.Kparentp >= 0 { // one.unit:80, go-struct-rio.act:233
			pos := 0
		...
		if va[0] == "tree_parallel" && len(va) > 1 && me.Kparentp >= 0 { // one.unit:81, go-struct-rio.act:233
			pos := 0
	        ...
	        fmt.Printf("?No its %s for Thought %s,%s > one.unit:78, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadMemo(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApThought[ len( act.ApThought )-1 ].ItsMemo = append(act.ApThought[ len( act.ApThought )-1 ].ItsMemo, st)	// one.unit:78, go-struct-rio.act:416
	name,_ := st.Names["memo_id"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Memo_" + name	// one.unit:109, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpMemo) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Memo > one.unit:108, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpMemo) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "parent" { // one.unit:78, go-struct-rio.act:570
		if me.Kparentp >= 0 {
	...
	if va[0] == "computation" {
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Memo %s,%s > one.unit:108, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func (me KpOps) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if (va[0] == "Memo_computation" && len(va) > 1) { // one.unit:115, go-struct-rio.act:706
		for _, st := range glob.Dats.ApMemo {
	...
	if va[0] == "previous" { // one.unit:125, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApOps[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Ops > one.unit:125, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Ops > one.unit:125, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpOps) DoIts(glob *GlobT, va []string, lno string) int {
	if (va[0] == "Memo_computation") { // one.unit:115, go-struct-rio.act:595
		for _, st := range glob.Dats.ApMemo {
			if (st.Kcomputationp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Ops %s,%s > one.unit:125, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func (me KpComp) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if va[0] == "parent" { // one.unit:139, go-struct-rio.act:621
		if (me.Kparentp >= 0 && len(va) > 1) {
			return( glob.Dats.ApComp[ me.Kparentp ].GetVar(glob, va[1:], lno) )
		}
	}
	if (va[0] == "Comp_parent" && len(va) > 1) { // one.unit:139, go-struct-rio.act:706
	...
	if (va[0] == "Element_comp" && len(va) > 1) { // one.unit:167, go-struct-rio.act:706
	...
	if va[0] == "previous" { // one.unit:133, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApComp[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Comp > one.unit:133, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Comp > one.unit:133, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpComp) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "Element" { // one.unit:149, go-struct-rio.act:685
	...
	if (va[0] == "Comp_parent") { // one.unit:139, go-struct-rio.act:595
		for _, st := range glob.Dats.ApComp {
			if (st.Kparentp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
	...
	if (va[0] == "Element_comp") { // one.unit:167, go-struct-rio.act:595
		for _, st := range glob.Dats.ApElement {
			if (st.Kcompp == me.Me) {
				if len(va) > 1 {
					ret := st.DoIts(glob, va[1:], lno)
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Comp %s,%s > one.unit:133, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadElement(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApComp[ len( act.ApComp )-1 ].ItsElement = append(act.ApComp[ len( act.ApComp )-1 ].ItsElement, st)	// one.unit:133, go-struct-rio.act:416
	name,_ := st.Names["name"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Element_" + name	// one.unit:151, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpElement) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if va[0] == "comp" { // one.unit:167, go-struct-rio.act:621
		if (me.Kcompp >= 0 && len(va) > 1) {
			return( glob.Dats.ApComp[ me.Kcompp ].GetVar(glob, va[1:], lno) )
		}
	}
	if (va[0] == "parent") { // one.unit:133, go-struct-rio.act:585
		if (me.Kparentp >= 0 && len(va) > 1) {
			return( glob.Dats.ApComp[ me.Kparentp ].GetVar(glob, va[1:], lno) );
		}
	}
	if va[0] == "previous" { // one.unit:149, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApElement[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Element > one.unit:149, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Element > one.unit:149, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpElement) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "Opt" { // one.unit:181, go-struct-rio.act:685
	...
	if va[0] == "parent" { // one.unit:133, go-struct-rio.act:570
		if me.Kparentp >= 0 {
			st := glob.Dats.ApComp[ me.Kparentp ]
			if len(va) > 1 {
				return( st.DoIts(glob, va[1:], lno) )
	...
	if va[0] == "comp" {
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Element %s,%s > one.unit:149, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}
...
func loadOpt(act *ActT, ln string, pos int, lno string, flag []string, names map[string]any) int {
	...
	act.ApElement[ len( act.ApElement )-1 ].ItsOpt = append(act.ApElement[ len( act.ApElement )-1 ].ItsOpt, st)	// one.unit:149, go-struct-rio.act:416
	name,_ := st.Names["name"].(string)
	s := strconv.Itoa(st.Kparentp) + "_Opt_" + name	// one.unit:187, go-struct-rio.act:464
	act.index[s] = st.Me;
	st.MyName = name
...
func (me KpOpt) GetVar(glob *GlobT, va []string, lno string) (bool, string) {
	if (va[0] == "parent") { // one.unit:149, go-struct-rio.act:585
		if (me.Kparentp >= 0 && len(va) > 1) {
			return( glob.Dats.ApElement[ me.Kparentp ].GetVar(glob, va[1:], lno) );
		}
	}
	if va[0] == "previous" { // one.unit:181, go-struct-rio.act:178
		if (me.Me > 0 && len(va) > 1) {
			return( glob.Dats.ApOpt[ me.Me - 1 ].GetVar(glob, va[1:], lno) )
		}
	}
	if len(va) > 1 {
		msg := fmt.Sprintf("?%s.?:%s,%s,Opt > one.unit:181, go-struct-rio.act:185?", va[0], lno, me.LineNo)
		return false, msg
	}
	if va[0] == "payload" {
		tmp := maps.Clone(me.Names)
	...
	r := me.Names[va[0]]
	if r == nil { 
		rr := fmt.Sprintf("?%s?:%s,%s,Opt > one.unit:181, go-struct-rio.act:197?", va[0], lno, me.LineNo) 
		return false,rr
	}
	rr := me.Names[va[0]].(string)
	return true,rr
...
func (me KpOpt) DoIts(glob *GlobT, va []string, lno string) int {
	if va[0] == "parent" { // one.unit:149, go-struct-rio.act:570
		if me.Kparentp >= 0 {
		...
		return(0)
	        ...
	        fmt.Printf("?No its %s for Opt %s,%s > one.unit:181, go-struct-rio.act:222?", va[0], lno, me.LineNo)
		glob.RunErrs += 1
	return(0)
}

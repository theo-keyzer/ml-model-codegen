...
*/
func parse(glob *GlobT, winp int, body interface{}, lno string ) (bool,string) {
	return strs(glob,winp,body,lno,true,true)
}
...
func addCmd(glob *GlobT, winp int, cmd *KpAdd, lno string, body interface{}) (int) {
	...
	} else if contains(cmd.Flags, "depth") {
		kData = winp
	} else {
...
*/	
	...
	dat := rec.Dat
	if contains(cmd.Flags, "parse") {
		new_winp := winp
		if len(va) > 2 {
			new_path := strings.Split(va[2], ".")
			new_rec := getPath(glob, glob.Winp, new_path, cmd.LineNo)
			new_winp,_ = new_rec.Dat.(int)
		}
		_, kData = strs(glob,new_winp,kData,lno,true,true)
		// _, kData = parse(glob,new_winp,kData,lno)
	}
	if slice, ok := dat.([]interface{}); ok {

...
func strs(glob *GlobT, winp int, ss interface{}, lno string, prErr, isErr bool) (bool, string) {
	...
	for i < n {
		if runes[i] == '$' {
			// Handle $$ escape sequence first — highest priority
			if i+1 < n && runes[i+1] == '$' {
			...
			// Only if it's not $$ do we check for ${...}
			if i+1 < n && runes[i+1] == '{' {
				// Find closing brace
				start := i + 2
				end := start
				...
				if end == n {
					// No closing brace → treat everything from $ as literal
					out.WriteString(string(runes[i:]))
					break
				}

				inside := string(runes[start:end])
				parts := strings.SplitN(inside, ":", 2) // only split on first :
				varPath := strings.Split(parts[0], ".")

				resOk, res := sGetVar(glob, winp, parts, varPath, lno)
				if !resOk {
					ok = false
					if prErr {
						fmt.Println(res)
